...
2. Sol review 前已用显式路径暂存任务文件；用 `guard.py audit --repo <仓库>` 对照 init 时的 Git baseline、staged index、WRITE 与当前实际改动，拦截瞒报、index/worktree 分叉、越界文件和常见密钥；未被本任务触碰的既有脏文件保持在外。状态文件、日志和一次性诊断产物不得暂存。
3. 核对任务要求涉及的源码、构建产物、安装副本或发布版本。只有内容一致性属于成功条件时才计算一次哈希；输入、版本和提交未变化时复用同一运行证据，不跨阶段重复计算。Git 使用显式路径；无 Git 仓库或证据不足就如实报告。
4. 所有有 WRITE 的任务都必须已有配置匹配、结果为 pass、且绑定当前任务 Diff 指纹的 Sol xhigh `record-review`；FAST 小任务也不例外。返工或任何后续改动后必须重新 Verify 和重新 review。
